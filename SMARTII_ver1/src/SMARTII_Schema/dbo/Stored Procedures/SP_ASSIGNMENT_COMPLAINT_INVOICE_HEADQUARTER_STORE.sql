...
-- Description:	轉派案件查詢(總部、門市、廠商)歷程:反應單，依照以下條件先篩選一次
-- 連絡者、被反應者、反映單號、反應類別，一對多的資料在C# 二查後過濾
-- =============================================
CREATE PROCEDURE [dbo].[SP_ASSIGNMENT_COMPLAINT_INVOICE_HEADQUARTER_STORE]
...
-- Insert statements for procedure here
	...
	CACI.ID AS IdentityID,
	CACI.INVOICE_ID AS InvoiceID,
	CACI.INVOICE_TYPE AS InvoiceType,
	CACI.TYPE AS Type,
	...
	CACI.NOTICE_DATETIME AS NoticeDateTime,
	CACI.CONTENT AS NoticeContent,
	...
	Inner Join CASE_ASSIGNMENT_COMPLAINT_INVOICE CACI on C.CASE_ID =CACI.CASE_ID
	'
	...
	IF (@Type IS NOT NULL)
	BEGIN
	SET @where = @where+' and CACI.TYPE ='+ CONVERT(varchar, @Type)
	END
	--案件編號
	...
	BEGIN
	SET @where = @where+' and CACI.NOTICE_DATETIME >='''+ CONVERT(varchar, @NoticeDateStarTime)++''' and CACI.NOTICE_DATETIME <='''+ CONVERT(varchar, @NoticeDateEndTime)+''''
	END
	--通知內容
	IF(@NoticeContent IS NOT NULL and @NoticeContent != '')
	BEGIN
	SET @where = @where+' and CACI.CONTENT like ''%'+  @NoticeContent +'%'''
	END
	--案件內容
	...
	'or C.[CASE_ID] in (select distinct ac.[CASE_ID]  from [dbo].[CASE] ac inner  join [dbo].[CASE_ASSIGNMENT_COMPLAINT_INVOICE_USER] ca on ca.CASE_ID = ac.CASE_ID 
