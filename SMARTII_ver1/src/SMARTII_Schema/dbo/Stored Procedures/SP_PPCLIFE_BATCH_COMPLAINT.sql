...
-- Description:	統一藥品 客訴紀錄，二次查詢，再查詢出序號、客訴品牌、商品名稱、國際條碼、批號、轉派人員、回覆內容
--需要二次查詢是因為資料有經過編碼後儲存、商品需要經過特殊解密
-- =============================================
CREATE PROCEDURE [dbo].[SP_PPCLIFE_BATCH_COMPLAINT]
...
BEGIN
	...
	Inner Join QUESTION_CLASSIFICATION QC on C.QUESION_CLASSIFICATION_ID = QC.ID
	'
	...
	BEGIN
	set @where = @where + ' and C.QUESION_CLASSIFICATION_ID in ('+@ClassificationID+')'
	END
	--全部 = 0、未結案 = 1
	IF (@Type IS NOT NULL)
	BEGIN
	IF (@Type = 1)
	BEGIN
	set @where = @where + ' and (C.CASE_TYPE = 0 or C.CASE_TYPE = 1)'
	END
	END
