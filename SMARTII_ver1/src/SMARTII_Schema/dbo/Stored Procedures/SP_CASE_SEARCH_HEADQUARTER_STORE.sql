...
-- Description:	案件查詢(總部、門市) 依照以下條件先篩選一次
-- 連絡者、被反應者、案件標籤、反映單號、結案處置，一對多的資料在C# 二查後過濾
-- =============================================
CREATE PROCEDURE [dbo].[SP_CASE_SEARCH_HEADQUARTER_STORE]
...
DECLARE @Complained nvarchar(MAX)
...
BEGIN
	...
	END
	--組織節點
	IF (@HeadNodeID IS NOT NULL)
	BEGIN
	SET @where = @where+'and C.[CASE_ID] in (SELECT distinct c.[CASE_ID] FROM [dbo].[CASE] c 
	inner join  [dbo].[CASE_COMPLAINED_USER] cc on c.CASE_ID = cc.CASE_ID and (cc.UNIT_TYPE =1 OR cc.UNIT_TYPE = 2) and cc.NODE_ID in('''+ @HeadNodeID +'''))'
	END
