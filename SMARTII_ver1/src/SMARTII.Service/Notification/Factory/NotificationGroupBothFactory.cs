...
{
    public class NotificationGroupBothFactory : NotificationGroupBaseFactory, INotificationGroupFactory
    {
        ...
        public NotificationGroupBothFactory(ICommonAggregate CommonAggregate,
        ...
        {
            _CommonAggregate.Logger.Info($"【提醒群組通知】 《問題分類/商品行為》");
            _CommonAggregate.Logger.Info($"【提醒群組通知】  計算是否達標");

            // 此次達標的總案件
            var allActualCases = cases.Where(x => x.Items.Any(g => g.ItemID == group.ItemID) &&
                                               x.QuestionClassificationID == group.QuestionClassificationID).ToList();
            ...
            using (var scope = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
            {
                ...
                if (group.IsNotificated == false && isArrive)
                {
                    ...
                    var target = $@"{questionClassificationParentNames}\{group.ItemName}";

                    // 組合通知物件
                    var personalNotifications = GeneratePersonalNotifications(group, actualCases, target);
        ...
        private void cancelGroup(NotificationGroup group)
        {
            ...
            // 修改標的狀態
            var con = new MSSQLCondition<NOTIFICATION_GROUP>(y => y.ID == group.ID);
            con.ActionModify(x =>
            {
                x.ACTUAL_COUNT = 0;
