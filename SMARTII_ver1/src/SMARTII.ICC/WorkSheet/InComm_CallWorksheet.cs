...
namespace SMARTII.ICC
{
    public class InComm_CallWorksheet : IWorksheet
    {
        public void CreateWorksheet(XLWorkbook book, object payload, object extend = null)
        {
            var data = (List<InComm_CallHistory>)payload;
            ...
            foreach (var item in data)
            {
                ...
                ws.Cell(row, column + 4).Value = "'" + item.CardNumber;
                ...
                ws.Cell(row, column + 9).Value = "'" + item.FinishDate;
