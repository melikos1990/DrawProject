...
namespace SMARTII.MisterDonut.Controllers
{
    [Authentication]
    [RoutePrefix("Api/MisterDonut/Report")]
    public class MisterDonut_ReportController : BaseApiController
    {
        private readonly ICommonAggregate _CommonAggregate;
        private readonly IMisterDonutFactory _MisterDonutFactory;

        public MisterDonut_ReportController(ICommonAggregate CommonAggregate, IMisterDonutFactory MisterDonutFactory)
        {
            this._CommonAggregate = CommonAggregate;
            this._MisterDonutFactory = MisterDonutFactory;
        }
        ...
        public async Task<HttpResponseMessage> GetReport(string DateRange)
        {
            try
            {
                ...
                var bytes = await _MisterDonutFactory.GetOnCallExcel(s.Value, e.Value);
                ...
                response.Content.Headers.ContentDisposition =
                   new ContentDispositionHeaderValue("attachment") { FileName = "多拿滋來電紀錄-S5.xlsx".Encoding() };
