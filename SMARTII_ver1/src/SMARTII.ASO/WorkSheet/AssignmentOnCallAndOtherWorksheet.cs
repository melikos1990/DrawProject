...
{
    public class AssignmentOnCallAndOtherWorksheet : IWorksheet
    {
        public void CreateWorksheet(XLWorkbook book, object payload, object extend = null)
        {
            ...
            ws.Cell(1, column + 10).Value = "結案日期";
            ws.Cell(1, column + 11).Value = "案件狀態";
            ws.Cell(1, column + 12).Value = "立案時間(A)";
            ws.Cell(1, column + 13).Value = "轉派時間(B)";
            ws.Cell(1, column + 14).Value = "轉派內容";
            ws.Cell(1, column + 15).Value = "轉派單位回覆時間(C)";
            ws.Cell(1, column + 16).Value = "轉派回覆內容";
            ws.Cell(1, column + 17).Value = "結案時間(D)";
            ws.Cell(1, column + 18).Value = "非轉派案件處理時間(D-A)";
            ws.Cell(1, column + 19).Value = "轉派單位處理時間(C-B)";
            ws.Cell(1, column + 20).Value = "滿意度調查";
            ws.Range(1, 1, 1, column + 20).Style.Fill.BackgroundColor = XLColor.FromArgb(192, 192, 192);
            #endregion
            int row = 2;
            foreach (var item in data)
            {
                ...
                ws.Cell(row, column + 12).Value = "'" + item.CreatDateTime;
                ws.Cell(row, column + 13).Value = "'" + item.AssignmentCreatDateTime;
                ws.Cell(row, column + 14).Value = "'" + item.AssignmentContent;
                ws.Cell(row, column + 15).Value = "'" + item.AssignmentFinishDateTime;
                ws.Cell(row, column + 16).Value = "'" + item.AssignmentResume;
                ws.Cell(row, column + 17).Value = "'" + item.CaseFinishDateTime;
                ws.Cell(row, column + 18).Value = "'" + item.WithoutAssignmentProcessDateTime;
                ws.Cell(row, column + 19).Value = "'" + item.AssignmentProcessDateTime;
                ws.Cell(row, column + 20).Value = "'" + item.Satisfaction;
                row++;
            }
            //左右至中
            ws.Range(1, 1, row - 1, column + 20).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);

            //問題、回覆至左
            ws.Range(2, column + 6, row - 1, column + 7).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Left);

            //水平 至中
            ws.Range(1, 1, row - 1, column + 20).Style.Alignment.SetVertical(XLAlignmentVerticalValues.Center);

            //字體大小
            ws.Range(1, 1, row - 1, column + 20).Style.Font.FontSize = 10;
            //格線
            var rngTable3 = ws.Range(1, 1, row - 1, column + 20);
