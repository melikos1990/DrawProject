...
{
    public class PPCLifeNotificationNoBatchNo : PPCLifeNotificationBase, IPPCLifeNotificationFactory
    {
        private readonly ICaseAggregate _CaseAggregate;
        private readonly ICommonAggregate _CommonAggregate;

        public PPCLifeNotificationNoBatchNo(ICaseAggregate CaseAggregate, ICommonAggregate CommonAggregate)
        {
        ...
        public List<PPCLifeEffectiveSummary> Execute(List<CasePPCLife> casePPCLives)
        {
            ...
            {           
                ...
                {
                    if (rule.ItemID == caseItem.ItemID && (caseItem.GetParticular<PPCLIFE_CaseItem>().BatchNo ?? "") == "")
                    {
                        count += 1;
                ...
                if (count >= 5)
                {
                    ...
                    {
                        PPCLifeArriveType = PPCLifeArriveType.NothingBatchNo,
                        ItemID = rule.ItemID,
                        BatchNo = "",
            ...
            _CommonAggregate.Logger.Info($"【統藥大量叫修通知】  無批號同產品 規則比對完畢。");
