...
{
    public class OnCallAndOtherWorksheet : IWorksheet
    {
        public OnCallAndOtherWorksheet()
        {
        }
        ...
        public void CreateWorksheet(XLWorkbook book, object payload, object extend = null)
        {
            ...
            ws.Cell(1, column + 7).Value = "轉派單位人員";
            ws.Cell(1, column + 8).Value = "結案日期";
            ws.Cell(1, column + 9).Value = "回覆處理內容";
            ws.Range(1, 1, 1, column + 9).Style.Fill.BackgroundColor = XLColor.FromArgb(192, 192, 192);
            ...
            foreach (var item in data)
            {
                ...
                ws.Cell(row, column + 7).Value = "'" + item.AssignmentUserName;
                ws.Cell(row, column + 7).Style.Alignment.WrapText = true;
                ws.Cell(row, column + 8).Value = "'" + item.FinishDate;
                ws.Cell(row, column + 9).Value = "";
                row++;
            }
            //左右至中
            ...
            //字體大小
            ws.Range(1, 1, row - 1, column + 9).Style.Font.FontSize = 10;
            //格線
            var rngTable3 = ws.Range(1, 1, row - 1, column + 9);
