...
namespace SMARTII.ColdStone.Service
{
    public class ColdStoneFacade
    {
        ...
        public ColdStoneFacade(ICaseAggregate CaseAggregate, IOrganizationAggregate OrganizationAggregate, IMasterAggregate MasterAggregate, ReportFacade ReportFacade)
        {
        ...
        /// <returns></returns>
        public async Task<ColdStoneDataList> GetOnCallExcel(DateTime start, DateTime end, string nodekey)
        {
            //取得Nodekey範圍
            ...
            var pathway = headerList.Where(x => x.NodeKey == NodeKeyValue.PATHWAY).First().NodeID;
            ...
            // 客訴問題分類(含子項目)
            var generalQuestion = classList.Where(x => x.FirstCode == CodeValue.GENERAL_CS)?.Select(x => x.ID).ToList() ?? new List<int>();
            ...
            var result = new ColdStoneDataList();
            ...
            result.ColdStoneComplaint = CombinationComplaintData(caseComplainList, classList, generalQuestion);
        ...
        /// <param name="classList"></param>
        /// <returns></returns>
        private List<ColdStoneComplaintHistory> CombinationComplaintData(List<Case> data, List<QuestionClassification> classList, List<int> generalQuestion)
        {
            List<ColdStoneComplaintHistory> result = new List<ColdStoneComplaintHistory>();
            ...
            foreach (var item in data)
            {
                ...
                var temp = new ColdStoneComplaintHistory();
