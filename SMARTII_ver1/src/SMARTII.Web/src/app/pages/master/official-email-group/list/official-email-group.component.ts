...
import { OfficialEmailGroupSearchViewModel } from 'src/app/model/master.model';
...
export const PREFIX = 'OfficialEmailGroupComponent';

@Component({
  selector: 'app-official-email-group',
  templateUrl: './official-email-group.component.html',
  styleUrls: ['./official-email-group.component.scss']
})
export class OfficialEmailGroupComponent extends FormBaseComponent implements OnInit {
  ...
  public model: OfficialEmailGroupSearchViewModel = new OfficialEmailGroupSearchViewModel();
  ...
  btnAdd($event: any) {
    ...
    this.store.dispatch(new fromRootActions.RouteActions.changeRouteAction({
      url: './pages/master/official-email-group-detail',
      params: {
  ...
  onBtnDisable($event: any) {
    ...
    this.store.dispatch(new fromRootActions.AlertActions.alertOpenAction(this.getLoopQuestionMessage('是否停用?',
      () => {
        ...
        this.store.dispatch(new fromOfficialEmailGroupActions.disableAction(payload));
      }
    )));
  ...
  initializeTable() {
    ...
    this.ajax.url = 'Master/OfficialEmailGroup/GetList/'.toHostApiUrl();
    ...
    this.columns = [
      ...
      {
        text: this.translateService.instant('OFFICIAL_EMAIL_GROUP.ACCOUNT'),
        name: 'Account',
        disabled: true,
        order: 'ACCOUNT'
      },
      {
        text: this.translateService.instant('OFFICIAL_EMAIL_GROUP.USERS'),
        name: 'UserNames',
        disabled: true,
        order: '',
        customer : true
      ...
      {
        text: this.translateService.instant('OFFICIAL_EMAIL_GROUP.IS_ENABLED'),
        name: 'IsEnabled',
        disabled: true,
        order: 'IS_ENABLED',
      },
