...
import { NodeJobListViewModel, AddUserViewModel, VendorNodeDetailViewModel } from 'src/app/model/organization.model';
...
@Component({
  selector: 'app-vendor-user-table',
...
export class UserTableComponent extends FormBaseComponent implements OnInit, OnDestroy {
  ...
  public main: VendorNodeDetailViewModel = new VendorNodeDetailViewModel();
  ...
  subscription() {
    ...
    this.model$ = this.store.select(x => x.organization.vendorNode.job)
      .subscribe(job => {
        this.model = { ...job };
      });
    this.main$ = this.store.select(x => x.organization.vendorNode.detail)
      .subscribe(detail => {
        this.main = { ...detail };
        setTimeout(() => {
          this.store.dispatch(new fromVendorNodeActions.selectJobAction(this.getCurrentJob()));
        }, 0);
      });
  ...
  btnAddUser($event) {
    ...
    payload.success = () => {
      this.store.dispatch(new fromVendorNodeActions.loadDetailAction(new EntrancePayload<number>(this.model.NodeID)));
      this.modalService.dismissAll();
    };

    this.store.dispatch(new fromVendorNodeActions.addUserAction(payload));
  }
  ...
  btnDeleteUser($event) {
    this.store.dispatch(new fromRootActions.AlertActions.alertOpenAction(this.getLoopQuestionMessage('是否刪除?',
      () => {
        ...
        this.store.dispatch(new fromVendorNodeActions.deleteUserAction(payload));
      }
    )));
  }
