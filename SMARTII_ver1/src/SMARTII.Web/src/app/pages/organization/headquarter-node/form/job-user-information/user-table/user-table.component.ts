...
import { NodeJobListViewModel, AddUserViewModel, HeaderQuarterNodeDetailViewModel } from 'src/app/model/organization.model';
...
export class UserTableComponent extends FormBaseComponent implements OnInit, OnDestroy {
  ...
  public main: HeaderQuarterNodeDetailViewModel = new HeaderQuarterNodeDetailViewModel();
  ...
  subscription() {
    ...
    this.model$ = this.store.select(x => x.organization.headerQuarterNode.job)
      .subscribe(job => {
        this.model = { ...job };
      });
    this.main$ = this.store.select(x => x.organization.headerQuarterNode.detail)
      .subscribe(detail => {
        this.main = { ...detail };
        setTimeout(() => {
          this.store.dispatch(new fromHeaderQuarterNodeActions.selectJobAction(this.getCurrentJob()));
        }, 0);
      });
  ...
  btnAddUser($event) {
    ...
    payload.success = () => {
      this.store.dispatch(new fromHeaderQuarterNodeActions.loadDetailAction(new EntrancePayload<number>(this.model.NodeID)));
      this.modalService.dismissAll();
    };

    this.store.dispatch(new fromHeaderQuarterNodeActions.addUserAction(payload));
  }
  ...
  btnDeleteUser($event) {
    this.store.dispatch(new fromRootActions.AlertActions.alertOpenAction(this.getLoopQuestionMessage('是否刪除?',
      () => {
        ...
        this.store.dispatch(new fromHeaderQuarterNodeActions.deleteUserAction(payload));
      }
    )));
  }
