...
import { CallCenterNodeDetailViewModel, JobListViewModel, AddJobViewModel, NodeDefinitionSearchViewModel, NodeJobListViewModel } from 'src/app/model/organization.model';
...
@Component({
  selector: 'app-callcenter-job-table',
...
export class JobTableComponent extends FormBaseComponent implements OnInit, OnDestroy {
  ...
  jobRowSelect = ($event) => this.store.dispatch(new fromCallCenterNodeActions.selectJobAction($event.data));
    ...
    this.store.dispatch(new fromRootActions.AlertActions.alertOpenAction(this.getLoopQuestionMessage('是否刪除?',
      () => {
        ...
        payload.success = () => {
          ...
          this.store.dispatch(new fromCallCenterNodeActions.loadDetailAction(new EntrancePayload<number>(this.model.ID)));
          this.modalService.dismissAll();
        };
        this.store.dispatch(new fromCallCenterNodeActions.deleteJobAction(payload));
      }
    )));
  ...
  btnAddJob($event: JobListViewModel[]) {
    ...
    this.store.dispatch(new fromCallCenterNodeActions.addJobAction(payload));
  }
  ...
  initializeTable() {
    ...
    this.options = {
      columns: {
        Name: {
          title: this.translateService.instant('NODE_DEFINITION.NAME'),
          width: '20%',
        },
        IsEnabled: {
        ...
        Level: {
          title: this.translateService.instant('NODE_DEFINITION.LEVEL'),
          width: '20%',
        },
  ...
  btnUserPreview($event) {
    ...
    const ref = this.modalService.open(CallcenterNodeUserModalComponent, { size: 'lg', container: 'nb-layout', backdrop: 'static', windowClass: 'modal-xl' });
