...
namespace SMARTII._21Century
{
    public class OnComplaintWorksheet : IWorksheet
    {
        ...
        {
            var data = (List<_21CenturyComplaintHistory>)payload;
            //客訴紀錄
            ...
            ws.Cell(1, 5).Value = "區";
            ws.Cell(1, 6).Value = "客訴門市";

            int column = 6;
            //第二層開始
            for (int i = 2; i <= classificationCount; i++)
            {
                ws.Cell(1, 6 + i - 1).Value = "問題分類" + i.ToString();
                column++;
            }

            if (classificationCount < 2)
            {
                ws.Cell(1, 7).Value = "問題分類2";
                column++;
            }
            ...
            ws.Cell(1, column + 8).Value = "是否認列";


            int allColumn = column + 8;
            ws.Range(1, 1, 1, allColumn).Style.Fill.BackgroundColor = XLColor.FromArgb(192, 192, 192);
            ...
            foreach (var item in data)
            {
                ...
                ws.Cell(row, 5).Value = "'" + item.Zone;
                ws.Cell(row, 6).Value = "'" + item.ComplainedNodeName;
                //分類
                for (int i = 1; i < classificationCount; i++)
                {
                    if (item.ClassList == null || item.ClassList.ParentNamePathByArray.Count() < 2)
                        continue;
                    ws.Cell(row, 7 + i - 1).Value = "'" + item.ClassList.ParentNamePathByArray[i];
                ...
                ws.Cell(row, column + 8).Value = "'" + item.IsRecognize;
                row++;
            }
            //左右至中
